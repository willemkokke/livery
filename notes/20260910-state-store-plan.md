...
rows of a series that declares an age. The daily child runs the local
part unattended. A keyed ref the current matrix still produces is never
dropped, and every line says what went and why the rest stayed.
...
1. **The row layer.** `Series.rows`, `row`, and `put` with schema,
   ...
   CI-only write from a local run is refused.
...
4. **One janitor** (#404 folded in). Every declared series enumerated
   ...
   and `issue.sweep` folded into `fm janitor`; the daily child runs the
   local part; `fm issue.start` keeps running the worktree rule first.
5. **Reading by hand.** `fm store.ls` and `fm store.show`, read-only,
   through the same reader; the docs' scattered descriptions of the
   record, the store, the marks, and the timings become one section,
   "The state store", in `packages/workshop/docs/index.md`.

Slices 1 to 3 are a day together; 4 and 5 another.
