        ...
        def __call__(  # type: ignore[override]
            ...
            **flags: Any,
            ...
            Args:
                ...
                allow: Allow extra privileged entitlement (e.g., "network.host", "security.insecure", "device", "buildx.local.delete"). May be repeated: a list emits the flag once per item.
                annotation: Add annotation to the image. May be repeated: a list emits the flag once per item.
                attest: Attestation parameters (format: "type=sbom,generator=image"). May be repeated: a list emits the flag once per item.
                build_arg: Set build-time variables. May be repeated: a list emits the flag once per item.
                build_context: Additional build contexts (e.g., name=path). May be repeated: a list emits the flag once per item.
                builder: Override the configured builder instance. Defaults to `default`.
                cache_from: External cache sources (e.g., "user/app:cache", "type=local,src=path/to/dir"). May be repeated: a list emits the flag once per item.
                cache_to: Cache export destinations (e.g., "user/app:cache", "type=local,dest=path/to/dir"). May be repeated: a list emits the flag once per item.
                call: Set method for evaluating build ("check", "outline", "targets"). Defaults to `build`.
                cgroup_parent: Set the parent cgroup for the "RUN" instructions during build.
                check: Shorthand for "--call=check".
                compress: Compress the build context using gzip. Not available on Linux, Windows and macOS.
                cpu_period: Limit the CPU CFS (Completely Fair Scheduler) period. Not available on Linux, Windows and macOS.
                cpu_quota: Limit the CPU CFS (Completely Fair Scheduler) quota. Not available on Linux, Windows and macOS.
                cpu_shares: CPU shares (relative weight). Not available on Linux, Windows and macOS.
                cpuset_cpus: CPUs in which to allow execution (0-3, 0,1). Not available on Linux, Windows and macOS.
                cpuset_mems: MEMs in which to allow execution (0-3, 0,1). Not available on Linux, Windows and macOS.
                debug: Enable debug logging.
                file: Name of the Dockerfile. Defaults to `PATH/Dockerfile`.
                force_rm: Always remove intermediate containers. Not available on Linux, Windows and macOS.
                iidfile: Write the image ID to a file.
                isolation: Container isolation technology. Not available on Linux, Windows and macOS.
                label: Set metadata for an image. May be repeated: a list emits the flag once per item.
                load: Shorthand for "--output=type=docker".
                memory: Memory limit. Not available on Linux, Windows and macOS.
                memory_swap: Swap limit equal to memory plus swap: -1 to enable unlimited swap. Not available on Linux, Windows and macOS.
                ...
                no_cache: Do not use cache when building the image.
                no_cache_filter: Do not cache specified stages. May be repeated: a list emits the flag once per item.
                output: Output destination (format: "type=local,dest=path"). May be repeated: a list emits the flag once per item.
                platform: Set target platform for build. May be repeated: a list emits the flag once per item.
                policy: Policy configuration (format: `"filename=path[,filename=path][,reset=true|false][,disabled=true|false][,strict=true|false][,log-level=level]")`. May be repeated: a list emits the flag once per item.
                progress: Set type of progress output ("auto", "none", "plain", "quiet", "rawjson", "tty"). Defaults to `auto`.
                ...
                resource: Resource limits for build containers (format: "memory=2g", "cpu-quota=50000"). May be repeated: a list emits the flag once per item. Added in 29.6.0.
                rm: Remove intermediate containers after a successful build. Defaults to `True`. Not available on Linux, Windows and macOS.
                sbom: Shorthand for "--attest=type=sbom".
                secret: Secret to expose to the build (format: "id=mysecret[,src=/local/secret]"). May be repeated: a list emits the flag once per item.
                security_opt: Security options. Not available on Linux, Windows and macOS.
                shm_size: Shared memory size for build containers.
                ssh: SSH agent socket or keys to expose to the build (format: "default|<id>[=<socket>|<key>[,<key>]]"). May be repeated: a list emits the flag once per item.
                tag: Image identifier (format: "[registry/]repository[:tag]"). May be repeated: a list emits the flag once per item.
