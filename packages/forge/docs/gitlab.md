...
| `create_repo` | `POST /projects` | `visibility` from `private`; when the owner is a group, `namespace_id` is resolved first (`GET /namespaces?search=`); `initialize_with_readme` so the default branch exists, as the protocol requires; then, when the group (`GET /groups/:path`, `default_branch_protection`) or the instance (`GET /application/settings`, unreadable reads as protected) protects new default branches, `GET /projects/:path/protected_branches` until the rule a background job writes appears (60 s budget) and `DELETE /projects/:path/protected_branches/:branch`, because GitLab protects a new project's default branch on creation and the protocol hands the creator an open branch |
...
| `merge_now` | `PUT /projects/:path/merge_requests/:iid/merge` | `squash`, `squash_commit_message` from title and message, and `should_remove_source_branch` read from the project setting, because the setting alone only pre-fills the UI checkbox; GitLab answers 405 while the mergeability recompute runs, and 422 "Branch cannot be merged" for the beat after a pipeline succeeds while `detailed_merge_status` is still `checking`; both pass through verbatim, and a caller classifies them through `merge_hold` |
| `merge_hold` | `GET /projects/:path/merge_requests/:iid` | the `detailed_merge_status` field, verbatim |
